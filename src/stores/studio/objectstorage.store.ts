  ...
  implements IObjectStorageDataSource
{
  ...
  async getObjectHead(key: string): Promise<ObjectInfoDto> { 
    this.checkAll();
    return await fetchObjectHead({ providerId: this.providerId, bucket: this.bucket, key: key });
  }

  async getPresignedUrl(key: string, opts?: { ttl?: number; contentType?: string; disposition?: string;
  }): Promise<PresignedUrlDto> {
     this.checkAll();
    return await presignGet({ providerId: this.providerId, bucket: this.bucket, key: key, ...opts });
  }
