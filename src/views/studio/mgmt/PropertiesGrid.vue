<template>
    <PageToolbar :title="title" :label="subtitle" class="bg-transparent" :closeable="false" @refresh="refresh"
...
function emitAll(reason = 'change') {
    ...
    const filtered = all.filter((row) => normalizeName(row.name).length > 0)
    emit('change', filtered)               // 필요 시 후킹해서 사용
}
...
const onCellEditingStopped = (event: any) => {
    ...
    if (data?.isNew) data.isNew = false;
    validateRows();
};
...
async function addRow() {
    const newItem: PropertyRow = { name: "", value: "", isEdit: false, isNew: true, nameError: 'required' };
...
onBeforeUnmount(() => {
    const gui = (gridApi.value as any)?.getGui?.();
