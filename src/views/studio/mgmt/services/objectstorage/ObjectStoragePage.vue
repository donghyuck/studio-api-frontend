<template>
    ...
    <v-card density="compact" class="mt-5">
        ...
        <v-divider></v-divider> 
        <v-col cols="12" md="12">
            <GridContent ref="gridContentRef_Objects" :rowData="gridDataForObjects" :columns="columnDef_Objects">
            </GridContent>
        </v-col>
    </v-card>
    <ObjectDialog v-model="obejctHeadDialog.visible" :bucket="bucket ?? undefined" :object-key="obejctHeadDialog.key" @close="obejctHeadDialog.visible = false"></ObjectDialog>  
</template>
...
const crumbs = computed<any[]>(() => buildBreadcrumb(prefix.value, { rootLabel: bucket.value?.bucket }) as any[]);
