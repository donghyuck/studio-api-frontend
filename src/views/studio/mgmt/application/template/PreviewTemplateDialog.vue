...
import { renderBody, renderSubject } from '@/data/studio/mgmt/template';
...
function buildRenderModel() {
    ...
    const model = fromRowData(rows as Property[]) as Record<string, string> | undefined;
    return model ?? {};
}
...
async function getData() {
    ...
    try {
        ...
        if (data) {
            properties.value = toRowData((data.properties ?? {}) as Record<string, string>);
            previewSubject.value = '';
            previewBody.value = '';
